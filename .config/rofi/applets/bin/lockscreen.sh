...
set_permanant() {
  set_wall
  line_to_edit="$(cat $startup_config_file | grep "exec --no-startup-id betterlockscreen")"
  sed -i "s|$line_to_edit|exec --no-startup-id betterlockscreen -u ${dir}${wallpaper} --fx dim|g" $startup_config_file
}
